...
function countSmileys(arr) {
    ...
    const smileArray = arr.filter(item => {
        if (item.match(mask)) {
            return item.match(mask);
        }
        if (item.match(maskTwo)) {
            return item.match(maskTwo);
        }
    });
...
In case of an empty array return 0. You will not be tested with invalid
 input (input will always be an array). Order of the face (eyes, nose, 
    mouth) elements will always be the same.

...
function josephus(items, k) {
    ...
    const getNum = (newArr, step) => {
        let i = 0;
        i += step;

        //if step > length of items
        ...
        let sortedArr = [...newArr.slice(i, newArr.length),...newArr.slice(0, i-1)];
        result['arr'] = [...sortedArr];
        return result;
    }

    let tmpObj = getNum(items, k);
    result.push(tmpObj.num);

    for (let i = 0; i < items.length-1; i++) {
        tmpObj = getNum(tmpObj.arr, k);
        result.push(tmpObj.num);
    }
